...
class BaseModel(ABC):
    ...
    # Default value so log_param/log_metric are safe to call before train()
    ...
    def evaluate(self, dataset) -> dict:
        """Compute classifier metrics on a tabular dataset.

        Args:
            dataset: TabularDataset instance.

        Returns:
            Dict with all metrics as key/float pairs.
        """
        X, y = dataset.X, dataset.y
        y_pred = self.predict(X)

        # Models like MLPClassifier return raw logits (N, num_classes) — convert to class labels
        if y_pred.ndim == 2:
            y_pred = y_pred.argmax(axis=1)

        accuracy = accuracy_score(y, y_pred)

        # Per-class precision and recall (one value per class, in label order)
        ...
        # Average precision and recall (macro = unweighted mean across classes)
        avg_precision = precision_score(y, y_pred, average="macro", zero_division=0)
        avg_recall = recall_score(y, y_pred, average="macro", zero_division=0)

        metrics = {
            "accuracy": float(accuracy),
            "spearmanr": float(np.nan_to_num(spearmanr(y, y_pred).statistic)),
            "precision": float(avg_precision),
            "recall": float(avg_recall),
        }

        # Add per-class metrics with class name as prefix, e.g. "setosa_precision"
        for class_name, precision, recall in zip(dataset.class_names, precision_per_class, recall_per_class):
            metrics[f"{class_name}_precision"] = float(precision)
            metrics[f"{class_name}_recall"] = float(recall)

        return metrics

    def to(self, device) -> BaseModel:
        """Move model to device. No-op for non-PyTorch models. Returns self for chaining."""
        return self
