...
Run preprocessing and training as standalone K8s Jobs on a local Docker Desktop cluster.
...
- A K8s Job runs a single container to completion, then stops (unlike a Deployment). You manually sequence stages (preprocess, then train)
